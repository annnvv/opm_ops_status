  ...
  df <- df |> 
    dplyr::mutate(status = ifelse(is.na(status) & weekend ==1, "Weekend", status)) 
  
  for(y in min(year(df$date)):max(year(df$date))){
    print(paste0("Year ", y), digits = 1)
    print(summary(df[df$year == y, "mean_snow_inch"]))
  }
  
  for(y in min(year(df$date)):max(year(df$date))){
    print(paste0("Year ", y), digits = 1)
    print(summary(df[df$year == y & df$winter == 1, "mean_snow_inch"]))
  }
  ...
  ggplot(aes(x = date, y = mean_snow_inch, color = status)) + 
    theme_bw() + 
    geom_point(size = 2) +
    ...
    labs(title = "Snow Fall and Government Operating Status", 
         ...
         subtitle = "Measured at IAD, BWI, and National Arboretum",
  ...
  seasons <- base::unique(df$season)
  seasons <- seasons[!is.na(seasons)]

  for(y in seasons){
    print(y)
    ggplot(df[df$season == y, ], aes(date, mean_snow_inch, color = status)) + 
      theme_bw() + 
      geom_point(size = 2) +
